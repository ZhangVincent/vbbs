...
func (aliyun *aliyunOssUploader) PutImage(data []byte, contentType string, filesize int64) (string, error) {
	if strs.IsBlank(contentType) {
		contentType = "image/jpeg"
	}
	key := generateImageKey(data, contentType)
	return aliyun.PutObject(key, data, contentType, 0)
}

func (aliyun *aliyunOssUploader) PutObject(key string, data []byte, contentType string, filesize int64) (string, error) {
...
func (aliyun *aliyunOssUploader) CopyImage(originUrl string) (string, error) {
	...
	return aliyun.PutImage(data, contentType, 0)
}
